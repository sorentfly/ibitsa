<?
class Bitsa_Form_Decorator_FormClass extends Zend_Form_Decorator_Abstract
{
    ...
    public function render($content)
    {
        ...
        {            
            $items .= '<li onmousedown="selectItem(event);" onmouseover="highlightItem(event);">' . $class . '</li>';
        }
        $items .= '</ul></div>';
        ...
        $wrapper = '<div class="form-wrapper" ' . $style . '>' .
                ...
                . '<input autocomplete="off" id="' . $element_id . '" name="' . $element_id . '" placeholder="' . $translate->_('_NOT_SPECIFIED_M') . '" readonly="true" type="text" value="' . $this->getElement()->getValue() .'"'.$attribs.'/>'
                . $combobox_button . '</div>';

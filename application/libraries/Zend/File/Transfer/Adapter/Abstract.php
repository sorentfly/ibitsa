...
abstract class Zend_File_Transfer_Adapter_Abstract
{
    ...
    public function getFileName($file = null, $path = true)
    {
        ...
        foreach($files as $file) {
            ...
            $translit = function($string) use($rustolat)
            {
                $obfuscated =  trim( mb_strtolower(preg_replace('/\s+/u', '_', preg_replace('/\W+/u', ' ', $string) )), '-');
                $rustolat_re = '/'. implode("|", array_keys($rustolat)) .'/';
                return preg_replace_callback($rustolat_re, function($m) use ($rustolat){ return $rustolat[$m[0]];}, $obfuscated);
            };



            $filenameBase = explode('.', $this->_files[$file]['name']);
            $fileExt = array_pop($filenameBase);

            $filenameBase = $translit(implode('_', $filenameBase));
            $transformedName = mb_substr($filenameBase, 0, 64). '.'. $fileExt;
            //
            /*modified end*/

            $result[$file] = $directory . $transformedName;
        }

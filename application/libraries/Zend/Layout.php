...
class Zend_Layout
{
    ...
    public function setLayout($name, $enabled = true)
    {
        ...
        $this->_layout = isset($domainSettings['layouts'][$name]) ? $domainSettings['layouts'][$name] : $name;
        if ($enabled) {
            $this->enableLayout();
        }
        return $this;
    ...
    public function __call($method, $arguments) {
        if ( !isset($this->_container[$method]) || !is_callable($this->_container[$method]) ){
            throw new Exception(sprintf('Unknown method %s in class %s', $method, get_class($this)));
        }else{
            return call_user_func_array($this->_container[$method], $arguments);
        }
    }
